...
std::vector<std::unique_ptr<PlayableEvent>> makePlayables() {
	...
	e.Set(HLVR_EventKey_SimpleHaptic_Where_Regions_UInt32s, region.data(), region.size());
...
TEST_CASE("The events system works", "[EventSystem]") {
	...
	SECTION("HasKey will return true if the key is present") {
		event.Set(HLVR_EventKey_SimpleHaptic_Effect_Int, 122);
		REQUIRE(event.HasKey(HLVR_EventKey_SimpleHaptic_Effect_Int));
	}
...
TEST_CASE("Validation machinery should work") {
	SECTION("If a key is not present, it isn't an error (using validate, because we support optional)") {
		...
		auto result = validate<int>(HLVR_EventKey_SimpleHaptic_Effect_Int, data, [](int effect) { return true; });
		REQUIRE(!result);

	}
	SECTION("If a key is present, it also isn't an error") {
		ParameterizedEvent data;
		data.Set(HLVR_EventKey_SimpleHaptic_Effect_Int, 1);
		auto result = validate<int>(HLVR_EventKey_SimpleHaptic_Effect_Int, data, [](int effect) { return true; });
		REQUIRE(!result);
	...
	SECTION("If a key is not present, it returns an error (using validate_required)") {
		ParameterizedEvent data;
		auto result = validate<int>(HLVR_EventKey_SimpleHaptic_Effect_Int, data, [](int effect) { return true; }, key_required);
	...
	SECTION("If the key has a value but it's the wrong type, it should return error") {
		ParameterizedEvent data;
		data.Set(HLVR_EventKey_SimpleHaptic_Effect_Int, 1.0f);
		auto result = validate<int>(HLVR_EventKey_SimpleHaptic_Effect_Int, data, [](int effect) { return true; });
	...
	SECTION("If the key has a value but it fails the constraint, it should return error") {
		ParameterizedEvent data;
		data.Set(HLVR_EventKey_SimpleHaptic_Effect_Int, 1);
		auto result = validate<int>(HLVR_EventKey_SimpleHaptic_Effect_Int, data, [](int effect) { return false; });
...
TEST_CASE("Higher level event validation should work") {
	...
	SECTION("A BasicHapticEvent should generate the correct errors.. [smoketest]") {
		ParameterizedEvent data;
		data.Set(HLVR_EventKey_SimpleHaptic_Effect_Int, 0); //invalid value
		data.Set(HLVR_EventKey_SimpleHaptic_Duration_Float, 2); //invalid type
		data.Set(HLVR_EventKey_SimpleHaptic_Strength_Float, 1.0f); //valid

		playable->debug_parse(data, &result);
		REQUIRE(result.Count == 2);
		
		auto foundEffectErr = std::find_if(std::begin(result.Errors), std::end(result.Errors), 
			[](const HLVR_Event_KeyParseResult& result) { return result.Key == HLVR_EventKey_SimpleHaptic_Effect_Int; });
		...
		auto foundDurationErr = std::find_if(std::begin(result.Errors), std::end(result.Errors),
			[](const HLVR_Event_KeyParseResult& result) { return result.Key == HLVR_EventKey_SimpleHaptic_Duration_Float; });
		...
		auto foundStrengthErr = std::find_if(std::begin(result.Errors), std::end(result.Errors),
				[](const HLVR_Event_KeyParseResult& result) { return result.Key == HLVR_EventKey_SimpleHaptic_Strength_Float; });
